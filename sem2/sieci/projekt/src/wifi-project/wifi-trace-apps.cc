...
NS_LOG_COMPONENT_DEFINE ("WifiTraceApps");

TypeId Sender::GetTypeId(void) {
    static TypeId
            tid =
                    TypeId("Sender") .SetParent<Application> () .AddConstructor<
                            Sender> () .AddAttribute("PacketSize",
                            "The size of packets transmitted.", UintegerValue(
                                    64), MakeUintegerAccessor(
                                    &Sender::m_pktSize), MakeUintegerChecker<
                                    uint32_t> (1)) .AddAttribute("Destination",
                            "Target host address.", Ipv4AddressValue(
                                    "255.255.255.255"),
                            MakeIpv4AddressAccessor(&Sender::m_destAddr),
                            MakeIpv4AddressChecker()) .AddAttribute("Port",
                            "Destination app port.", UintegerValue(1603),
                            MakeUintegerAccessor(&Sender::m_destPort),
                            MakeUintegerChecker<uint32_t> ()) .AddAttribute(
                            "NumPackets", "Total number of packets to send.",
                            UintegerValue(300), MakeUintegerAccessor(
                                    &Sender::m_numPkts), MakeUintegerChecker<
                                    uint32_t> (1)) .AddAttribute("Interval",
                            "Delay between transmissions.",
                            RandomVariableValue(ConstantVariable(7.0)),
                            MakeRandomVariableAccessor(&Sender::m_interval),
                            MakeRandomVariableChecker()) .AddTraceSource("Tx",
...
void Sender::StartApplication() {
    ...
    if (m_socket == 0) {
        Ptr<SocketFactory> socketFactory =
                GetNode()->GetObject<SocketFactory> (
...
void Sender::SendPacket() {
    ...
    NS_LOG_INFO("Sending packet " << packet->GetUid() << " at "
            << timestamp.GetTimestamp() << " to " << m_destAddr);
...
TypeId Receiver::GetTypeId(void) {
    static TypeId tid =
            TypeId("Receiver") .SetParent<Application> () .AddConstructor<
                    Receiver> () .AddAttribute("Port", "Listening port.",
                    UintegerValue(1603),
                    MakeUintegerAccessor(&Receiver::m_port),
                    MakeUintegerChecker<uint32_t> ()) .AddTraceSource("Rx",
                    "A new packet is received", MakeTraceSourceAccessor(
                            &Receiver::m_rxTrace));
    return tid;
}
...
void Receiver::SetCounter(Ptr<CounterCalculator<> > calc) {
    m_calc = calc;
...
void Receiver::Receive(Ptr<Socket> socket) {
    ...
    while (packet = socket->RecvFrom(from)) {
        ...
        if (InetSocketAddress::IsMatchingType(from)) {
            ...
            NS_LOG_INFO("Received packet " << packet->GetUid() << " at " << now
                    << " (tx=" << tx << ") from " << address.GetIpv4());
        }
...
TypeId TimestampTag::GetTypeId(void) {
    static TypeId tid =
            TypeId("TimestampTag") .SetParent<Tag> () .AddConstructor<
                    TimestampTag> () .AddAttribute("Timestamp",
                    "Some momentous point in time!", EmptyAttributeValue(),
